                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::status ret1, ret2;;
    
    getattr(inum, st);
    ret1 = yfs->setattr(inum, st, attr);
    if (ret1 != yfs_client::OK)
    {
      std::cout << "set attr failed" << std::endl;
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  ret = yfs->read(file_inum, size, st, off, buf);
  if (ret != yfs_client::OK)
  {
                 ...
                 struct fuse_file_info *fi)
{
  ...
  ret = yfs->write(file_inum, buf, st, size, off);
  if (ret != yfs_client::OK)
  {
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  ret1 = yfs->create(parent, name, file_inum);  
    
  if (ret1 != yfs_client::OK) { return ret1; }
  e->ino = file_inum;
  // In yfs, timeouts are always set to 0.0, and generations are always set to 0
  ret2 = getattr(e->ino, e->attr);

  if (ret2 != yfs_client::OK) { return ret2; }
  // You fill this in for Lab 2
  return yfs_client::OK; 
}
...
{
  std::cout << "lookup get called for " << name << std::endl;
  struct fuse_entry_param e;
  ...
  ret = yfs->lookup(dir_inum, name, file_inum);  
  if (ret == yfs_client::OK)
  {
    found = true;
  }
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  std::vector<yfs_client::dirent> dir_contents;
  
  yfs->readdir(inum, dir_contents);
  for (size_t i=0; i<dir_contents.size(); i++)
  {
    dirbuf_add(&b, dir_contents[i].name.c_str(), dir_contents[i].inum);
  }
     ...
     mode_t mode)
{
  ...
  ret = yfs->mkdir(parent_inum, name, dir_inum);
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  ret = yfs->unlink(parent_inum, name);
